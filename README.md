...
- **[P1684](https://github.com/kokkos/mdspan)**: mdarray: I'm not yet sure I fully appreciate the motivating use cases. I could easily see adding explicit constructors and assignment operators which used an mdarray; however, I do not think I would implement a tensor in terms of an mdarray. I'm not saying tensors couldn't be implemented in terms of an mdarray or another owning multi-dimensional coontainer and perhaps there is value in it with more effort and thought. Currently, I just see it as an unnecessary abstraction which complicates and constrains the interface between the tensor and the memory layer.
...
- mdspan::at(...): Frankly, I don't have a strong use case for this; however, it seemed natural for a container which provides operator\[\](...) to also provide at(...) which performs additional index bound checking. I could also see C++ migrating in a different direction for safe memory access and essentially making these functions depricated.
