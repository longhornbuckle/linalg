...
template < class ThisIndexType, size_t Extent >
class extents< ThisIndexType, Extent >
#if !defined(_MDSPAN_USE_ATTRIBUTE_NO_UNIQUE_ADDRESS)
  : private detail::__no_unique_address_emulation<
      detail::__partially_static_sizes_tagged<detail::__extents_tag, ThisIndexType , size_t, Extent>>
#endif
{
public:
...
// internal typedefs which for technical reasons are public
  using __storage_t = detail::__partially_static_sizes_tagged<detail::__extents_tag, index_type, size_t, Extent>;
...
// private members dealing with the way we internally store dynamic extents
 private:
  ...
  std::size_t _static_extent_impl(size_t n, std::integer_sequence<size_t, Idxs...>) noexcept {
    return _MDSPAN_FOLD_PLUS_RIGHT(((Idxs == n) ? Extent : 0), /* + ... + */ 0);
  }
...
public:
  ...
  */
  ...
  MDSPAN_INLINE_FUNCTION
  static constexpr rank_type rank() noexcept { return rank_type(1); }
  MDSPAN_INLINE_FUNCTION
  static constexpr rank_type rank_dynamic() noexcept { return rank_type(Extent == dynamic_extent); }
  ...
  MDSPAN_TEMPLATE_REQUIRES(
    ...
    /* requires */ (
      ...
      decltype(detail::_check_compatible_extents(
        std::integral_constant<bool, 1 == sizeof...(OtherExtents)>{},
...
#endif
  {
    ...
    /* requires */ (
      ...
      ((sizeof...(Integral) == detail::_count_dynamic_extents<Extent>::val) ||
       (sizeof...(Integral) == 1))
      )
    )
#else
...
#endif
  MDSPAN_INLINE_FUNCTION
  constexpr extents(Integral... exts) noexcept
#if defined(_MDSPAN_USE_ATTRIBUTE_NO_UNIQUE_ADDRESS)
    : __storage_{
#else
  ...
  MDSPAN_TEMPLATE_REQUIRES(
    ...
    /* requires */ (
      ...
      ((N == detail::_count_dynamic_extents<Extent>::val) ||
       (N == 1))
    )
  )
#else
  ...
  size_t static_extent(size_t n) noexcept {
    ...
    // assert(n<rank());
    return _static_extent_impl(n, std::make_integer_sequence<size_t, 1>{});
  }
...
public:  // (but not really)
  ...
  extents __make_extents_impl(detail::__partially_static_sizes<index_type, size_t,Extent>&& __bs) noexcept {
  ...
  MDSPAN_FORCE_INLINE_FUNCTION
  constexpr
  operator ThisIndexType() noexcept { return __extent<0>(); }
